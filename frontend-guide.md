...
The frontend is split in 3 different applications:
...
All applications use the same stack, share the same tools, build processes, guidelines, architecture, and produce the same type of output.
...
Teacher and Student application are both [Single Page Apps](https://en.wikipedia.org/wiki/Single-page_application).
...
After the fist load, all interactions and route change will not trigger a page reload unless on very explicit operation (logout, change school, ...).
...
All interactions should be thought be used by a touch device.
Trackpads on ChromeBook is not easy to master and our users have difficulties being precise with it. That's why a lot of ChromeBook users cannot use the drawing features in Math questions.
...
All build steps are written in a bash file at the root of each project call `build.sh`.
It contains all the steps to create the different output of each web applications.
The build usually includes:
- Converting SASS to CSS
- Bundling CSS
- Versionifying CSS bundle
- Running Flow type check
- Running ESLint
- Bundling JS
- Uglyfying JS bundle
- Versionifying JS bundle
- Bundling JS vendor files
- Uglyfying JS vendor bundle
- Versionifying JS vendor bundle
- Running the test
...
Each build type (`dev` or `prod`) use a different cache file for this process (`browserify-cache-development.json`).
...
When calling an API resource, we use Backbone Collections and Models to transform them into usable, type-checked, javascript usuable class.
Every resource is atomically defined by it's model. Each model contains a `parse` function validating and transforming data from API response.

_Example of the model parse function_
```javascript
parse: function(response: Object): Object {
  // Validating the API response contains the keys we expect and the values are the type we expect
...
For each model we define the type of the return of the `get` function for each valid key. Calling `get` on a non valid `key` should throw a flow error, same for using the value as the incorrect type.
...
[Flow](https://flow.org/) is a tool that add a layer of typing to javascript code. It relies on annotations in the code to figure out which type are the elements. When running `flow check`, flow will check the code base and throw all typing errors found. Flow contains the definition of the javascript API and can be completed by passing definition of libraries we use, like React, Backbone or Lodash. Other library definition can be in [flow-typed](https://github.com/flowtype/flow-typed).
Importing an element to a file will also import its type but you can chose to only import its type.
...
By contention the first `describe` should contain the name of the tested file. Every nested `describe` should contain the name of the tested function.
Example:
```javascript
